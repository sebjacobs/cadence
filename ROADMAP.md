...
- **Ratings + key-aware selection** — enrich `music.db` with iTunes/Apple Music ratings and favourites, plus musical key (mixed-in-key / essentia / librosa). Use ratings to weight selection toward favourites and de-prioritise low-rated tracks; use key for harmonic adjacency across crossfades (Camelot wheel). Open question: rubberband preserves pitch so keys should survive retempo — verify before relying on it.
